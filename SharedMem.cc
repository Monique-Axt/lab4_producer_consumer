...
void SharedMem::allocate(key_t key, int size)
{
	...
	// shmget() returns an identifier for the shared mem segment associated with the key param
	if ((_id = shmget(key, size, S_IRUSR | S_IWUSR | IPC_CREAT)) == -1) {
		char msg[100];
...
{
    // the shmat() function attaches the shared mem segment specified by the shid param
    // to the address space of the calling process & increments the value of shm_nattach for chmid
    // success: returns the starting address of the segment
...
{
    // shmdt() detach the shared mem segment & decrements shm_nattach
	if ((shmdt(_mem)) == -1) {
...
int SharedMem::remove(void)
{
    ...
    // the interpretation of the buf param depends on the value of cmd, in this case:
    // IPC_RMID = remove shared mem seg shmid & destroy corresponding schmid_ds
